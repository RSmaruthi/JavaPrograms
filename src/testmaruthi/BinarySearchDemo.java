...
public class BinarySearchDemo {
    ...
    {
        result = BinarySearchDemo .search(a, 0, finalIndex, key);
        if (result >= 0)
 ...
 String key=util.scanner.next();
    int results;
    for (int keys = -4; keys < 6; keys++)
    {
        results = BinarySearchDemo . searchString(names, key);
        if (results >= 0)
            System.out.println(keys + " is at index " + results);
        else
            System.out.println(keys + " is not in the array.");
    }
